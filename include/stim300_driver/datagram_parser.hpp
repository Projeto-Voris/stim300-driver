...
struct DatagramParser {
  ...
  SensorConfig
  parseConfig(std::vector<uint8_t>::const_iterator &&buffer_itr) const;
...
private:
  ...
  double acc_scale_;
  double incl_scale_;
  void setDataScales(GyroOutputUnit gyro_o, AccOutputUnit acc_o,
  ...
  // which is 32 bits.
  static const uint32_t parseUnsigned(std::vector<uint8_t>::const_iterator &it,
                                      uint8_t size) {
    uint32_t tmp{0};
  ...
  parseTwosComplement(std::vector<uint8_t>::const_iterator &it,
                      const uint8_t size) {
