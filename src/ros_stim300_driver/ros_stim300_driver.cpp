...
Stim300DriverNode::Stim300DriverNode()
    : Node("stim300_driver_node"), gravity_(9.80665), sample_rate_(125) {
...
void Stim300DriverNode::timerCallback() {
  ...
  case Stim300Status::NEW_MEASURMENT:
    ...
    if (calibration_mode_ == true) {
      if (number_of_samples < NUMBER_OF_CALIBRATION_SAMPLES) {
        ...
        inclination_x_average =
            inclination_x_calibration_sum / NUMBER_OF_CALIBRATION_SAMPLES;
        inclination_y_average =
            inclination_y_calibration_sum / NUMBER_OF_CALIBRATION_SAMPLES;
        inclination_z_average =
            inclination_z_calibration_sum / NUMBER_OF_CALIBRATION_SAMPLES;

        average_calibration_roll =
            atan2(inclination_y_average, inclination_z_average);
        average_calibration_pitch =
      ...
      RPY.pitch = atan2(-inclination_x,
                        sqrt(pow(inclination_y, 2) + pow(inclination_z, 2)));
      ...
      if (acceleration_buffer_x.size() == 2 &&
          acceleration_buffer_y.size() == 2 &&
          acceleration_buffer_z.size() == 2) {
        if (std::abs(acceleration_buffer_x.back() -
                     acceleration_buffer_x.front()) < ACC_TOLERANCE ||
            dropped_acceleration_x_msg > MAX_DROPPED_ACC_X_MSG) {
        ...
        if (std::abs(acceleration_buffer_y.back() -
                     acceleration_buffer_y.front()) < ACC_TOLERANCE ||
            dropped_acceleration_y_msg > MAX_DROPPED_ACC_Y_MSG) {
        ...
        if (std::abs(acceleration_buffer_z.back() -
                     acceleration_buffer_z.front()) < ACC_TOLERANCE ||
            dropped_acceleration_z_msg > MAX_DROPPED_ACC_Z_MSG) {
        ...
        acceleration_buffer_x =
            std::vector<double>{acceleration_buffer_x.back()};
        acceleration_buffer_y =
            std::vector<double>{acceleration_buffer_y.back()};
        acceleration_buffer_z =
            std::vector<double>{acceleration_buffer_z.back()};
      } else {
        stim300msg_.linear_acceleration.x =
            driver_stim300_->getAccX() * gravity_;
        stim300msg_.linear_acceleration.y =
            driver_stim300_->getAccY() * gravity_;
        stim300msg_.linear_acceleration.z =
            driver_stim300_->getAccZ() * gravity_;
      }
      ...
      if (gyro_buffer_x.size() == 2 && gyro_buffer_y.size() == 2 &&
          gyro_buffer_z.size() == 2) {
        if (std::abs(gyro_buffer_x.back() - gyro_buffer_x.front()) <
                std::max(2 * std::abs(gyro_buffer_x.front()),
                         GYRO_X_PEAK_TO_PEAK_NOISE) ||
            dropped_gyro_x_msg > MAX_DROPPED_GYRO_X_MSG) {
        ...
        if (std::abs(gyro_buffer_y.back() - gyro_buffer_y.front()) <
                std::max(2 * std::abs(gyro_buffer_y.front()),
                         GYRO_Y_PEAK_TO_PEAK_NOISE) ||
            dropped_gyro_y_msg > MAX_DROPPED_GYRO_Y_MSG) {
        ...
        if (std::abs(gyro_buffer_z.back() - gyro_buffer_z.front()) <
                std::max(2 * std::abs(gyro_buffer_z.front()),
                         GYRO_Z_PEAK_TO_PEAK_NOISE) ||
  ...
  case Stim300Status::CONFIG_CHANGED:
    ...
    timer_->cancel();
    timer_ = this->create_wall_timer(
        std::chrono::milliseconds(static_cast<int>(1000.0 / (sample_rate_))),
        std::bind(&Stim300DriverNode::timerCallback, this));
    break;
